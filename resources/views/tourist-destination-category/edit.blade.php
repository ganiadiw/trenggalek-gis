<x-app-layout>
    <div class="py-4">
        <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
            <form class="px-8 py-6 mt-5 bg-white border-2 rounded-md shadow-lg" method="POST" action="{{ route('tourist-destination-categories.update', ['tourist_destination_category' => $touristDestinationCategory]) }}">
                @csrf
                @method('PUT')
                ...
                <div class="grid mb-5 gap-x-5 md:grid-cols-2">
                    <x-input-default-form type="text" name="name" :value="old('name', $touristDestinationCategory->name)" id="name" labelTitle="Nama Kategori*" error='name' placeholder="Wisata Pantai"></x-input-default-form>
                </div>
