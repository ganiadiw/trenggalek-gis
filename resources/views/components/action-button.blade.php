...
<div x-data="{ initial: true, deleting: false }" class="flex items-center text-sm">
    ...
    <div
        ...
        class="absolute flex space-x-3 scale-100 right-3 xl:right-8"
    >
        <span class="px-5 py-1 text-sm text-gray-200 bg-gray-600 rounded-md">@lang('Hapus ') <span class="font-semibold underline">{{ $value }}</span> @lang('?')</span>
        ...
        <form x-on:submit="$dispatch('deleting')" class="space-x-1" method="post" action="{{ $action }}">
            ...
            <button
                x-on:click.prevent="deleting = false; setTimeout(() => { initial = true })"
                x-on:deleting.window="$el.disabled = true"
